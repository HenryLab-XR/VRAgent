...
def evaluate_oracle_coverage(
    ...
    for bug in oracles:
        ...
        results.append({
            ...
            "trigger_condition": bug.get("trigger_condition", ""),
            "trigger_object": bug.get("trigger_object", ""),
            "trigger_action": bug.get("trigger_action", ""),
            "triggered": triggered,
        })
...
def collect_console_logs_from_iteration_logs(iteration_logs_path: str) -> List[str]:
    """Collect all console log strings from iteration_logs.json.

    Each iteration entry has a ``bugs`` field containing console log lines.
    This also captures any ``console_logs`` field if present.
    """
    data = load_json(iteration_logs_path)
    ...
    for entry in data:
        ...
        for field in ("bugs", "console_logs"):
            ...
            if isinstance(val, list):
                logs.extend(str(s) for s in val)
